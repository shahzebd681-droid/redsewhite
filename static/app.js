...
async function loadPublicSettings(){
  try{const r=await api('/api/public?nocache='+Date.now(),{cache:'no-store'});const d=await r.json();if(!r.ok)throw new Error(d.error||'Unable to load payment settings.');cfg=d.settings||{};renderDropdown();renderPaymentMethods();renderContacts();selectMethod('upi');}
