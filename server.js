...
const defaults = {
  ...
  min: '100',
  max: '100000',
  message: 'Please pay only within the minimum and maximum limits.',
  telegram: '',
  whatsapp: '',
  qr: ''
};
...
if (!db.prepare('SELECT id FROM admins LIMIT 1').get()) {
  ...
  console.log(
    `Admin created: ${username}. Set ADMIN_USER and ADMIN_PASSWORD before production.`
  );
}
...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    cb(
      null,
      /^image\/(png|jpeg|webp|gif)$/.test(file.mimetype)
    );
  }
});
  ...
  (req, res) => {
    try {
      ...
      if (
        ...
        return res.status(400).json({
          error: `Amount must be between ₹${s.min} and ₹${s.max}.`
        });
      }
      ...
      do {
        ...
        db
          .prepare('SELECT 1 FROM transactions WHERE code=?')
          .get(ref)
      );
...
app.listen(PORT, () => {
  console.log(
    `RedseWhite running on http://localhost:${PORT}`
  );
});
