...
async function requireCustomer(req, res, next) {
  try {
    ...
    return res
      .status(500)
      .json({ error: "Unable to verify customer session." });
  }
}
...
function publicCustomer(customer) {
  return {
    ...
    user_id: customer.user_id,
    // API keeps the simple names for the future Admin UI,
    // while the database uses the actual schema names.
...
async function ensureBuckets() {
  ...
  const shots = await supabase.storage.createBucket(SCREENSHOT_BUCKET, {
    public: false,
  });
  ...
  const qrPublic = await supabase.storage.updateBucket(QR_BUCKET, {
    public: true,
  });
...
app.post("/api/login", (req, res) => {
  ...
  setCookie(res, ADMIN_COOKIE, token, 8 * 60 * 60);
...
app.get("/api/me", (req, res) => {
  const authenticated = verifyAdminToken(getCookie(req, ADMIN_COOKIE));
...
app.post("/api/customer/login", async (req, res) => {
  try {
    ...
    if (!verifyCustomerPassword(password, customer.password_hash)) {
      return res.status(401).json({
        error: "Invalid User ID or password.",
      });
    }

    // Keep the schema's login timestamp updated without blocking login
    // if the timestamp update itself fails.
    await supabase
      .from("rw_customers")
    ...
    setCookie(res, CUSTOMER_COOKIE, token, 24 * 60 * 60);

    const temporary = customer.temporary_password === true;
...
app.get("/api/customer/me", async (req, res) => {
  try {
    ...
    res.json({
      authenticated: true,
      customer: publicCustomer(customer),
      requires_password_reset: customer.temporary_password === true,
    });
  } catch (error) {
...
app.post("/api/customer/change-password", requireCustomer, async (req, res) => {
  try {
    ...
    const validOldPassword = verifyCustomerPassword(
      oldPassword,
...
app.get("/api/admin/customers/:id", auth, async (req, res) => {
  try {
    ...
    console.error("ADMIN CUSTOMER ERROR:", error);
...
app.post("/api/admin/customers", auth, async (req, res) => {
  try {
    ...
    // Accept both API-friendly names and the database column names.
    const whatsapp = String(
      req.body?.whatsapp ?? req.body?.whatsapp_number ?? ""
    ).trim();

    const telegram = String(
      req.body?.telegram ?? req.body?.telegram_id ?? ""
    ).trim();

    if (!clientName || !userId || !temporaryPassword) {
      return res.status(400).json({
        error: "Client Name, User ID and Temporary Password are required.",
      });
    }
    ...
    const passwordHash = hashCustomerPassword(temporaryPassword);
    ...
    const duplicate =
      error?.code === "23505" || /duplicate|unique/i.test(message);
...
app.post("/api/admin/customers/:id/reset-password", auth, async (req, res) => {
  try {
    ...
    res.status(500).json({
      error: error.message || "Could not reset customer password.",
    });
  }
});
...
app.put("/api/admin/customers/:id/status", auth, async (req, res) => {
  try {
    ...
    res.status(500).json({
      error: error.message || "Could not update customer status.",
    });
  }
});
...
app.delete("/api/admin/customers/:id", auth, async (req, res) => {
  try {
    ...
    res.status(500).json({
      error: error.message || "Could not delete customer.",
    });
  }
});
...
app.post("/api/payment", upload.single("screenshot"), async (req, res) => {
  try {
    ...
    if (!utr || utr.length > 100) {
      return res.status(400).json({
        error: "Valid UTR/Transaction ID is required.",
      });
    }

    /* Global UTR duplicate protection for the existing transactions table. */
    const { data: duplicateUtr, error: utrError } = await supabase
      .from("transactions")
      .select("id")
      .eq("utr", utr)
      .maybeSingle();
...
app.get("/api/status/:code", async (req, res) => {
  try {
    ...
    res.json({
      ...data,
      created_at_ist: ist(data.created_at),
      updated_at_ist: ist(data.updated_at),
    });
  } catch (error) {
...
app.get("/api/admin/transactions", auth, async (req, res) => {
  try {
    ...
    res.json(
      (data || []).map((row) => ({
        ...row,
        created_at_ist: ist(row.created_at),
        updated_at_ist: ist(row.updated_at),
      }))
    );
  } catch (error) {
...
app.get("/api/admin/transactions/:id/screenshot", auth, async (req, res) => {
  try {
    ...
    const signed = await supabase.storage
      ...
      .createSignedUrl(data.screenshot, 10 * 60);
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!["qr_a", "qr_b", "qr_c"].includes(method)) {
        return res.status(400).json({
          error: "Invalid QR method.",
        });
      }
...
app.put("/api/admin/transactions/:id", auth, async (req, res) => {
  try {
    ...
    if (!["pending", "processing", "confirmed", "rejected"].includes(status)) {
      return res.status(400).json({
...
async function start() {
  try {
    ...
    app.listen(PORT, () => {
      console.log(`RedseWhite/Redwallet server running on port ${PORT}`);
    });
  } catch (error) {
