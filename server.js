...
// Keep the backend root private: send visitors to the customer site.
app.get('/', (req, res) => res.redirect(302, 'https://redsewhite-customer.onrender.com'));
...
function makeToken(type, subject, hours, extra = {}) {
...
function customerAuth(req, res, next) {
  ...
  if (data.force_reset === true) return res.status(403).json({ error: 'Password reset required.', force_reset: true });
  req.customerId = Number(data.sub);
  next();
}

function customerResetAuth(req, res, next) {
  const data = verifyToken(getCookie(req, CUSTOMER_COOKIE), 'customer');
  if (!data) return res.status(401).json({ error: 'Customer authentication required.' });
  if (data.force_reset !== true) return res.status(403).json({ error: 'This password reset session is no longer valid.' });
  req.customerId = Number(data.sub);
  next();
}
...
app.post('/api/customer/login', async (req, res) => {
  try {
    ...
    if (!verifyCustomerPassword(password, data.password_hash)) return res.status(401).json({ error: 'Invalid User ID or password.' });

    const needsReset = data.temporary_password === true;
    const rememberMe = req.body?.remember_me === true || String(req.body?.remember_me || '').toLowerCase() === 'true';
    const now = new Date().toISOString();
    const sessionHours = rememberMe ? 24 * 30 : 24;
    const token = needsReset ? makeToken('customer', data.id, 1, { force_reset: true }) : makeToken('customer', data.id, sessionHours);
    if (!needsReset) await supabase.from('rw_customers').update({ last_login_at: now, updated_at: now }).eq('id', data.id);

    const maxAge = needsReset ? 3600 : sessionHours * 3600;
    res.setHeader('Set-Cookie', `${CUSTOMER_COOKIE}=${token}; Max-Age=${maxAge}; ${customerCookieOptions()}`);
    res.json({
      ok: true,
      force_reset: needsReset,
      message: needsReset ? 'Please reset your temporary password before accessing the dashboard.' : undefined,
      customer: cleanCustomer({ ...data, last_login_at: needsReset ? data.last_login_at : now })
    });
  } catch (error) {
...
app.post('/api/customer/change-password', customerResetAuth, async (req, res) => {
  try {
    ...
    const token = makeToken('customer', req.customerId, 24);
    res.setHeader('Set-Cookie', `${CUSTOMER_COOKIE}=${token}; Max-Age=86400; ${customerCookieOptions()}`);
    res.json({ ok: true, force_reset_complete: true, authenticated: true });
  } catch (error) {
...
app.get('/api/admin/customers', auth, async (req, res) => {
  try {
    ...
    let q = supabase.from('rw_customers')
      .select('id,client_name,user_id,whatsapp_number,telegram_id,status,temporary_password,created_at,updated_at,last_login_at', { count: 'exact' })
      .order('id', { ascending: false });
    if (search) {
      const safe = search.replace(/[,%()]/g, ' ').trim();
      if (safe) q = q.or(`client_name.ilike.%${safe}%,user_id.ilike.%${safe}%`);
    }
    const { data, error, count } = await q.range(offset, offset + limit - 1);
    if (error) throw error;
    res.json({ customers: (data || []).map(cleanCustomer), pagination: pageMeta(page, limit, count || 0) });
  } catch (error) {
...
function parsePageParams(req, defaultLimit=10, maxLimit=50) {
...
function pageMeta(page, limit, total) {
  ...
  return { page, limit, total: Number(total || 0), pages, has_prev: page > 1, has_next: page < pages };
}

// Database uses the Supabase CHECK values white_inr / white_usdt.
// The API keeps the cleaner public keys inr / usdt_trc20.
const FUN_COIN_DB = { inr: 'white_inr', usdt_trc20: 'white_usdt' };
function funCoinDbType(coin) { return FUN_COIN_DB[String(coin || '').toLowerCase()] || null; }
function funCoinApiType(coin) {
  const value = String(coin || '').toLowerCase();
  return value === 'white_inr' ? 'inr' : value === 'white_usdt' ? 'usdt_trc20' : value;
}
function coinDisplayName(coin) {
  const value=String(coin||'').toLowerCase();
  return value==='white_inr'?'White INR Fun Coin':value==='white_usdt'?'White USDT Fun Coin (TRC20)':(coin||'—');
}
function paymentMethodDisplayName(method) {
  const value=String(method||'').toLowerCase();
  return ({upi:'UPI Payment',qr_a:'QR Payment A',qr_b:'QR Payment B',qr_c:'QR Payment C',bank:'Bank Transfer'})[value] || (method || '—');
}

async function ensureWalletSeeds() {
  const now = rwDate();
  const coinSeeds = [
    { coin_type: 'white_inr', rate: 1.20 },
    { coin_type: 'white_usdt', rate: 120.00 }
  ];
...
async function getRates() {
  ...
  if (error) throw error;
  return (data || []).reduce((a, r) => { a[funCoinApiType(r.coin_type)] = Number(r.rate); return a; }, {});
}
...
app.get('/api/customer/dashboard', customerAuth, async (req, res) => {
  try {
    ...
    const { page, limit, offset } = parsePageParams(req, 10, 50); const txRows = summary.transactions || []; const txPage = txRows.slice(offset, offset + limit); res.json({ customer: cleanCustomer(customer), balance: summary.balance, total_deposited: summary.total_deposited, total_used: summary.total_used, pending_amount: summary.pending_amount, rates, destinations: (destinations || []).map(d=>({...d,destination_type:(['inr','inr_bank','bank'].includes(String(d.destination_type||'').toLowerCase())?'inr':d.destination_type)})), payment_methods: paymentMethods, transactions: txPage.map(t=>({...t,payment_method_name:paymentMethodDisplayName(t.payment_method),coin_name:coinDisplayName(t.coin_type),destination_name:t.destination_type==='usdt_trc20'?'USDT TRC20':t.destination_type==='inr'?'INR Bank Account':(t.destination_type||'—')})), transactions_pagination: pageMeta(page, limit, txRows.length) });
...
app.get('/api/customer/transactions', customerAuth, async (req,res)=>{
  try {
    ...
    const { page, limit, offset } = parsePageParams(req, 10, 50);
    const {data,error,count}=await supabase.from('rw_transactions').select('*',{count:'exact'}).eq('customer_id',req.customerId).order('id',{ascending:false}).range(offset,offset+limit-1);
    if(error)throw error;
    res.json({transactions:data||[],pagination:pageMeta(page,limit,count||0)});
...
app.post('/api/customer/destinations', customerAuth, async (req,res)=>{
  try {
    ...
    const baseRow={customer_id:req.customerId,destination_type:type,label:label||null,account_holder_name:type==='inr'?holder:null,account_number:type==='inr'?account:null,ifsc:type==='inr'?ifsc:null,wallet_address:type==='usdt_trc20'?wallet:null,created_at:rwDate(),updated_at:rwDate()};
    const destinationTypes=type==='inr'?['inr','inr_bank','bank']:['usdt_trc20'];
    let data=null,lastError=null;
    for(const dbType of destinationTypes){
      const r=await supabase.from('rw_customer_destinations').insert({...baseRow,destination_type:dbType}).select('*').single();
      if(!r.error){data=r.data;break;}
      lastError=r.error;
      const msg=String(r.error.message||'').toLowerCase();
      if(type!=='inr'||!msg.includes('destination_type'))break;
    }
    if(!data)throw lastError||new Error('Could not save destination.');
    await audit('destination_create','customer',req.customerId,req.customerId,null,{destination_type:type,label});
    res.status(201).json({...data,destination_type:type});
  } catch(e){console.error('DEST CREATE ERROR:',e);res.status(500).json({error:e?.message||'Could not save destination.'});}
});
app.put('/api/customer/destinations/:id', customerAuth, async (req,res)=>{
  try {
    ...
    if(type==='inr'){row.account_holder_name=String((req.body?.account_holder_name ?? old.account_holder_name ?? '')).trim();row.account_number=String((req.body?.account_number ?? old.account_number ?? '')).trim();row.ifsc=String((req.body?.ifsc ?? old.ifsc ?? '')).trim().toUpperCase();row.wallet_address=null;if(!row.account_holder_name||!row.account_number||!row.ifsc)return res.status(400).json({error:'Account holder, account number and IFSC are required.'});}
    else if(type==='usdt_trc20'){row.wallet_address=String((req.body?.wallet_address ?? old.wallet_address ?? '')).trim();row.account_holder_name=null;row.account_number=null;row.ifsc=null;if(!row.wallet_address)return res.status(400).json({error:'USDT TRC20 wallet address is required.'});}
    else return res.status(400).json({error:'Invalid destination type.'});
    const dbTypes=type==='inr'?[String(old.destination_type||'').toLowerCase(),'inr','inr_bank','bank'].filter((v,i,a)=>v&&a.indexOf(v)===i):['usdt_trc20'];
    let data=null,lastError=null;
    for(const dbType of dbTypes){const r=await supabase.from('rw_customer_destinations').update({...row,destination_type:dbType}).eq('id',id).eq('customer_id',req.customerId).select('*').single();if(!r.error){data=r.data;break;}lastError=r.error;if(type!=='inr'||!String(r.error.message||'').toLowerCase().includes('destination_type'))break;}
    if(!data)throw lastError||new Error('Could not update destination.');
    await audit('destination_update','customer',req.customerId,req.customerId,null,{destination_id:id});res.json({...data,destination_type:type});
  } catch(e){console.error('DEST UPDATE ERROR:',e);res.status(500).json({error:e?.message||'Could not update destination.'});}
});
...
app.post('/api/customer/conversions', customerAuth, async(req,res)=>{
  try{
    const c=await requireCustomerActive(req,res);if(!c)return;
    const coin=String(req.body?.coin_type||'').toLowerCase();
    const amount=roundMoney(Number(req.body?.amount));
    const destinationId=Number(req.body?.destination_id);
    if(!['inr','usdt_trc20'].includes(coin))return res.status(400).json({error:'Invalid Fun Coin type.'});
    if(!Number.isFinite(amount)||amount<=0)return res.status(400).json({error:'Enter a valid INR amount to convert.'});
    const dbCoin=funCoinDbType(coin);if(!dbCoin)return res.status(400).json({error:'Invalid Fun Coin type.'});
    const {data:rateRow,error:re}=await supabase.from('rw_fun_coin_settings').select('rate').eq('coin_type',dbCoin).maybeSingle();if(re)throw re;if(!rateRow)return res.status(400).json({error:'Conversion rate is not configured.'});
    const rate=Number(rateRow.rate);
    if(!Number.isFinite(rate)||rate<=0)return res.status(400).json({error:'Conversion rate is not configured.'});
    const quantity=Number((amount/rate).toFixed(6));
    if(!Number.isFinite(quantity)||quantity<=0)return res.status(400).json({error:'Conversion amount is too small for the current rate.'});
    const {data:dest,error:de}=await supabase.from('rw_customer_destinations').select('*').eq('id',destinationId).eq('customer_id',req.customerId).maybeSingle();if(de)throw de;if(!dest)return res.status(400).json({error:'Please select a saved destination.'});const normalizedDestType=(['inr','inr_bank','bank'].includes(String(dest.destination_type||'').toLowerCase())?'inr':dest.destination_type);if(normalizedDestType!==coin)return res.status(400).json({error:'Selected destination does not match the selected Fun Coin.'});
    const txId=rwTransactionId('CON');const now=rwDate();const snapshot={id:dest.id,destination_type:normalizedDestType,label:dest.label,account_holder_name:dest.account_holder_name,account_number:dest.account_number,ifsc:dest.ifsc,wallet_address:dest.wallet_address};
    const {data:tx,error}=await supabase.from('rw_transactions').insert({transaction_id:txId,customer_id:req.customerId,transaction_type:'conversion',amount,coin_type:dbCoin,applied_rate:rate,coin_quantity:quantity,destination_type:coin,destination_snapshot:snapshot,status:'pending',created_at:now,updated_at:now,rejection_reason:null}).select('*').single();if(error)throw error;
...
app.get('/api/admin/wallet/transactions', auth, async(req,res)=>{
  try{
    const { page, limit, offset } = parsePageParams(req, 10, 50);
    let q=supabase.from('rw_transactions').select('*',{count:'exact'}).order('id',{ascending:false});
    ...
    if(search){
      const safe=search.replace(/[,%()]/g,' ').trim();
      if(safe)q=q.or(`transaction_id.ilike.%${safe}%,utr.ilike.%${safe}%`);
    }
    const {data,error,count}=await q.range(offset,offset+limit-1);
    if(error)throw error;
    const rows=data||[];
    const ids=[...new Set(rows.map(t=>t.customer_id))];
    let customers=[];
    if(ids.length){
      const r=await supabase.from('rw_customers').select('id,client_name,user_id').in('id',ids);
      if(r.error)throw r.error;
      customers=r.data||[];
    }
    const map=new Map(customers.map(c=>[c.id,c]));
    const transactions=rows.map(t=>({...t,customer:map.get(t.customer_id)||null,payment_method_name:paymentMethodDisplayName(t.payment_method),coin_name:coinDisplayName(t.coin_type),destination_name:t.destination_type==='usdt_trc20'?'USDT TRC20':t.destination_type==='inr'?'INR Bank Account':(t.destination_type||'—'),created_at_ist:ist(t.created_at),updated_at_ist:ist(t.updated_at)}));
    res.json({transactions,pagination:pageMeta(page,limit,count||0)});
...
app.get('/api/admin/wallet/customer/:id', auth, async(req,res)=>{
  try{
    const id=Number(req.params.id);const c=await findCustomer(id);if(!c)return res.status(404).json({error:'Customer not found.'});
    const { page, limit, offset } = parsePageParams(req, 10, 50);
    const s=await walletSummary(id);
    const txRows=s.transactions||[];
    const transactions=txRows.slice(offset,offset+limit).map(t=>({...t,payment_method_name:paymentMethodDisplayName(t.payment_method),coin_name:coinDisplayName(t.coin_type),destination_name:t.destination_type==='usdt_trc20'?'USDT TRC20':t.destination_type==='inr'?'INR Bank Account':(t.destination_type||'—')}));
    res.json({customer:cleanCustomer(c),balance:s.balance,total_deposited:s.total_deposited,total_used:s.total_used,pending_amount:s.pending_amount,transactions,pagination:pageMeta(page,limit,txRows.length)});
  }catch(e){res.status(500).json({error:'Could not load customer wallet.'});}
});
