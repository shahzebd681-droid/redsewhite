...
app.post('/api/customer/login', async (req, res) => {
  try {
    ...
    const token = needsReset ? makeToken('customer', data.id, 1, { force_reset: true }) : makeToken('customer', data.id, 24);
    if (!needsReset) await supabase.from('rw_customers').update({ last_login_at: now, updated_at: now }).eq('id', data.id);

    res.setHeader('Set-Cookie', `${CUSTOMER_COOKIE}=${token}; Max-Age=${needsReset ? 3600 : 86400}; ${customerCookieOptions()}`);
    res.json({
      ok: true,
