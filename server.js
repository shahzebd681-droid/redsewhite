...
app.post('/api/customer/conversions', customerAuth, async(req,res)=>{
  try{
    ...
    const {data:rateRow,error:re}=await supabase.from('rw_fun_coin_settings').select('rate').eq('coin_type',dbCoin).maybeSingle();if(re)throw re;if(!rateRow)return res.status(400).json({error:'Conversion rate is not configured.'});const rate=Number(rateRow.rate);const amount=roundMoney(quantity*rate);
