...
// Keep the backend root private: customers use the separate static site.
...
app.get('/api/customer/dashboard', customerAuth, async (req, res) => {
  try {
    ...
    res.json({ customer: cleanCustomer(customer), balance: summary.balance, total_deposited: summary.total_deposited, total_used: summary.total_used, pending_amount: summary.pending_amount, rates, destinations: (destinations || []).map(d=>({...d,destination_type:(['inr','inr_bank','bank'].includes(String(d.destination_type||'').toLowerCase())?'inr':d.destination_type)})), payment_methods: paymentMethods, transactions: (summary.transactions || []).slice(0, 100).map(t=>({...t,payment_method_name:paymentMethodDisplayName(t.payment_method),coin_name:coinDisplayName(t.coin_type),destination_name:t.destination_type==='usdt_trc20'?'USDT TRC20':t.destination_type==='inr'?'INR Bank Account':(t.destination_type||'—')})) });
...
app.get('/api/admin/wallet/transactions', auth, async(req,res)=>{
  try{
    ...
    const ids=[...new Set(rows.map(t=>t.customer_id))];let customers=[];if(ids.length){const r=await supabase.from('rw_customers').select('id,client_name,user_id').in('id',ids);if(r.error)throw r.error;customers=r.data||[];}const map=new Map(customers.map(c=>[c.id,c]));res.json(rows.map(t=>({...t,customer:map.get(t.customer_id)||null,payment_method_name:paymentMethodDisplayName(t.payment_method),coin_name:coinDisplayName(t.coin_type),destination_name:t.destination_type==='usdt_trc20'?'USDT TRC20':t.destination_type==='inr'?'INR Bank Account':(t.destination_type||'—'),created_at_ist:ist(t.created_at),updated_at_ist:ist(t.updated_at)})));
